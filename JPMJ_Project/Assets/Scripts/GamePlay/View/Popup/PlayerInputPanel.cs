/**
 * PlayerInputPanel
 * ui panel, buttons for player to select
 * brandy added
 */
...
public class PlayerInputPanel : UIObject
{
    ...
    public PlayerInputButton btn_RyuuKyoku; // 流局
    ...
    void Start()
    {
        ...
        btn_Reach.SetTag(ResManager.getString("button_reach")); // 听牌
        btn_Agari.index = 4;
        btn_Agari.SetOnClick(OnClick_Agari);
        btn_Agari.SetTag(ResManager.getString("button_tsumo")); // 自摸
        btn_Nagashi.index = 5;
        btn_Nagashi.SetOnClick(Onclick_Nagashi);
        btn_Nagashi.SetTag(ResManager.getString("button_pass")); // 过
